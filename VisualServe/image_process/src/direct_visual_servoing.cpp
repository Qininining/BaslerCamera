...
{
    // std::cout << "\t\t\terror_s_::" << image_gray_current_.size() << image_gray_desired_.size() << std::endl;
    ...
    // std::cout << "\t\t\terror_s_::" << error_s_.size() << std::endl;
...
Mat Direct_Visual_Servoing::get_interaction_matrix_gray(Mat& image_gray, Mat& image_depth, Mat& Camera_Intrinsic)
{
    ...
    double Z_inv, alpha_inv = 1/(820e-6); //速度单位：mm
    ...
    for(int i = 0; i < image_gray.rows; i++)
    {
        ...
        for(int j = 0; j < image_gray.cols; j++)
        {
            ...
            x = ((double*)xy.data)[0];
            y = ((double*)xy.data)[1];
            Z_inv = 1.0/(image_depth.at<double>(i, j)+0.000001);
            I_x_temp = ((double*)I_x.data)[i*I_x.cols+j];
            I_y_temp = ((double*)I_y.data)[i*I_y.cols+j];
            ((double*)L_e.data)[cnt*6+0] = I_x_temp * alpha_inv;
            ((double*)L_e.data)[cnt*6+1] = I_y_temp * alpha_inv;
            ...
            ((double*)L_e.data)[cnt*6+5] = -(j-Camera_Intrinsic.at<double>(1, 2))*I_x_temp + (i-Camera_Intrinsic.at<double>(0, 2))*I_y_temp;
            cnt++;        
        }
    }
    // 负号：转换到眼在手外。
...
{
    // I_x = get_image_gradient_x(image) * Camera_Intrinsic.at<double>(0, 0); //dI/ dx
    // I_y = get_image_gradient_y(image) * Camera_Intrinsic.at<double>(1, 1); //dI/ dy
    I_x = get_image_gradient_x(image); //dI/ du
    I_y = get_image_gradient_y(image); //dI/ dv
}
