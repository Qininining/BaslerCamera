...
Visual_Servoing::Visual_Servoing(int resolution_x, int resolution_y)
{
...
Mat Visual_Servoing::get_camera_velocity()
{
    ...
    // std::cout << "\t\t\titeration_num_::" << iteration_num_ << std::endl;

    Mat L_e_T = this->L_e_.t();                         // 转置
    Mat L_e_T_L_e = L_e_T * this->L_e_;                 // A^T * A
    Mat L_e_T_L_e_inv;
    invert(L_e_T_L_e, L_e_T_L_e_inv, DECOMP_SVD);       // 求逆
    Mat L_e_left_inv = L_e_T_L_e_inv * L_e_T;           // 左伪逆

    this->camera_velocity_ = -this->lambda_ * L_e_left_inv * this->error_s_;
    return this->camera_velocity_;
}
...
bool Visual_Servoing::is_success()
{
    ...
    double error = error_ave.at<double>(0,0);

    qDebug() << "\t\t\terror_ave:" << error << "\t" << this->epsilon_;

    if(error < this->epsilon_)
	{
